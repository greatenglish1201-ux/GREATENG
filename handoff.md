...
- **중등 출제 표준 정비됨**: 중등 예상문제는 `exam_authoring_middle_handoff.md` + `exam_gate_middle.py`로 고등처럼 강제 진행(교차 힌트 차단·매 문항 체크리스트 보고). 출제 착수 시 두 파일 먼저 읽을 것.
...
- **중등 출제 게이트·핸드오프 정비 — 고등처럼 강제화 (2026-06-29)**
  ...
  - **신규 `exam_gate_middle.py`**(고등 exam_gate.py의 중등판). 종료코드 0=PASS/1=FAIL, 통과 전 산출금지 동일철학.
    - 치명(FAIL): 배점합≠100·번호비연속·선지≠5·정답기호비정상·축약·해설출처누락.
    - ★경고(PASS여도 확인): **[7차★] 교차 힌트(정답누출)** — 동일문장 4어절+ 중복 / 단일어휘빈칸 정답어의 타문항 노출. [6차]개수형 정답=해설 정합. [8차]번호기호형 하단선지 군더더기. [3차]정답분포 쏠림.
    - 검증완료: 제철중3 PASS(누출0), 제철중2 변환본 PASS(실신호 '#3 import→#2 노출' + #18·19·22 번호기호형 군더더기 정확 탐지). 기능어/문장빈칸 오탐 제거 정제 완료.
  - **신규 `exam_authoring_middle_handoff.md`**: 중등 강제 체크리스트. 핵심 = ①중등≠고등 차이표(어법은 교과서 ■언어형식에서만, 외부지문존 없음, 밑줄형+개수형 혼합) ②**교차 힌트 4대 유형·차단 원칙**(이번 제철중2/중3에서 실제발생) ③매 문항 강제 체크리스트+보고형식 ④게이트 사용법 ⑤PDF 재제작 메모(번호기호형 하단선지 자동생략, 제너레이터 변수캡처 버그 주의).
  - 이후 중등 출제 시작 시 Claude는 이 핸드오프 + exam_gate_middle.py를 먼저 읽고 고등처럼 매 문항 체크리스트를 강제 보고.
  - 산출: `/mnt/user-data/outputs/exam_gate_middle.py`, `exam_authoring_middle_handoff.md`. ⚠️레포 반영 시 루트에 추가(고등 핸드오프와 병렬).
- **제철중2 기말 예상문제 — 지문/선지 교차 힌트(정답누출) 검수·수정 PDF 재제작 (2026-06-29)**
  - 입력=이미지PDF(제철중_2학년_기말고사_영어_예상문제2.pdf, 26문항). 텍스트추출 0자라 pdftoppm 렌더링→육안 전사.
  - ⚠️ **제철중3과 별개 시험**(중2, 문화/규칙/싱가포르·프랑스·스위스/베트남/나일강 소재). 어법포인트=관계대명사 수일치·의문사+to부정사·get/help+목+to·분사·명사절 that.
  - **발견한 정답누출(심각)**: ①#1④'ketchup hides real taste'=#13 영작정답 노출. ②#20 정답(fine/throw away/remains)이 #19본문·#1②③·#2⑤에 다중노출. ③get+목+to '올바른형'이 #25ⓒ·#9ⓒ에 노출→#8④ 추론. ④의문사+to부정사 #9·#23·#24·#25·#26 도배. (중간)⑤#2③ lonely정의→#10ⓑ. ⑥#8⑤=#15② 동일문장. ⑦#6ⓐ=#17① 동일대화. ⑧#6ⓔ longest river→#21.
  - **수정**: 영영풀이(#2)·어휘쓰임(#1) 정답노출선지 교체, #8 정답을 get→분사(make→made)로 바꿔 get누출 차단, 어법 소재 전면분산(students/rules/boy·where/what/how/when), #12·#19 본문 정답어 추상화, #18 도입중복 제거, #6·#17 대화 교체.
  - **수정문항**: #1 #2 #6 #8 #12 #14 #17 #18 #19 #20 + 어법분산 #9·#24·#25·#26.
  - **자동검증 ALL PASS**: 26문항·100점·번호연속·정답분포 ①4②5③6④5⑤6·동일문장중복0·빈칸정답어 타문항노출0·어법8문항 정답유일.
  - **재제작 방식**: 원본이 HTML인쇄(about:blank) 형태 → jc2_data.json(전문항 구조화)→HTML(2단 레이아웃·밑줄·박스 복원)→playwright PDF. ⚠️빌더 버그교훈: 제너레이터 내 삼항식 변수캡처로 전문항이 마지막q로 복제됨 → 명시적 for루프로 수정.
  - 산출: `/mnt/user-data/outputs/제철중2_기말_수정본.pdf`(문제+정답해설), `제철중2_누출검수보고서.md`, `jechuljung2_2026_1f_exam_data.json`(빌더데이터, 재제작용).
  - **레이아웃 개선(2026-06-29 추가)**: 본문에 ①~⑤ 번호가 박힌 번호기호형(흐름무관#22·문장삽입#11·#19·어휘부적절#18)은 하단 '① ①' 반복선지가 공간낭비 → 렌더러에 자동생략 로직 추가(본문 ①~⑤ 5개↑ + 하단선지가 번호만 반복이면 choices 미출력, 발문이 '①~⑤ 중'으로 본문번호 직접지칭). 정답해설표는 번호 그대로 유지. PDF 재생성.
- **jechuljung3_2026_1f_exam.json — 누출수정본 재검증: 복수정답 1건+출처누락 12건 추가수정, 치명 0건 확정 (2026-06-29 3차)**
  - 2차 누출수정 후 '오류 0건까지 재검증' 요청 → 어법 8문항 문장별 정밀 재판정 + 복수정답/발문정합 전수.
  - **추가로 잡은 실오류**: ①#20 어형함정형으로 바꾼 보기 'inspired'가 'so inspired that(영감 어린·탁월한)'으로도 자연스러워 **복수정답 위험** → 보기에서 inspired 제거, 주어를 'The story was ~ to young people'로 고정해 inspiring 유일화(정답 ①). ②원본부터 [출처:] 표기 없던 해설 12건(#1·3·4·6·8·10·13·17·19·21·23·25) → source 기반 일괄 부착(게이트 6차 요건).
  - **정답분포 재균등**: #20 정답 환원으로 ①6쏠림 → #2(내용일치)를 ②위치 재배치 → ①5②5③6④5⑤5.
  - **최종 자동검증 ALL PASS**: JSON유효·배점100·번호연속·정답기호정합·선지5개·**동일문장중복0·어휘정답 타문항노출0·해설출처 전건구비·축약0**. 어법8문항 문장별 정오 재판정 전부 정답유일(가목적어/과거완료/분사후치/명사절if 4포인트 유지). 미수정 11문항 복수정답·발문정합 재확인.
  - 산출=`/mnt/user-data/outputs/jechuljung3_2026_1f_exam.json`(덮어쓰기). 누적 수정문항(2~3차 합산): #2 #4 #5 #7 #9 #11 #12 #14 #15 #16 #18 #20 #22 #24 #26 + 출처12건.
- **jechuljung3_2026_1f_exam.json — 지문/선지 중복 정답누출 제거 + 분포균등화 (2026-06-29 2차)**
  - 1차 검수 후 사용자 요청으로 **교차 답노출 전수 점검** → 심각·중간 누출 다수 발견·수정.
  - **근본원인**: 어법 8문항이 같은 예문 풀(cello make / instruments made / made it possible / arrangement 과거완료)을 돌려써, 한 문항의 '틀린=정답' 문장이 다른 문항엔 '올바른 형태'로 노출 → 비교만으로 정답 추론 가능.
  - **수정한 누출(심각)**: ①#7↔#16↔#18 'cello make out of an oil tank' 3회중복 → #7 전용화, #16·#18에서 제거. ②#7↔#9↔#14↔#18 'instruments made/making' → #14를 분사 전용, 나머지 소재분리. ③#4↔#9↔#16↔#18 가목적어 'made it/that possible' → #4 전용, 타문항 다른소재. ④#11본문 'treasure'·'inspiring music' 직노출 → #22(treasure)는 영영풀이형, #20(inspiring)은 어형함정형(inspiring vs inspired, 정답①→②)으로 변경, #11 본문에서 해당문장 제거.
  - **수정한 누출(중간)**: ⑤#9ⓔ↔#15② 'if did I believe' 반복 → 소재분리(where/what time 등). ⑥#15③↔#18ⓔ 'have/had heard' 반복 → 제거. ⑦#23↔#24④ 'Do you find it helpful' 반복 → #24④ 다른 어색대화로. ⑧#12↔#13 Gómez/Cateura 공유 → #12를 콘서트감상 단락으로 분리.
  - **부수정리**: #4ⓐ·#5(D) if...or not 모호성 제거(whether 논쟁 회피), 정답분포 ④7쏠림 → #2·#26(선지독립) ④→① 재배치.
  - **수정 문항(13개)**: #2 #4 #5 #7 #9 #11 #12 #14 #15 #16 #18 #20 #22 #24 #26. 어법 핵심4포인트(명사절if/과거완료/분사후치/가목적어it~to)는 유지.
  - **최종 자동검수 PASS**: 26문항·100점·번호연속·JSON유효·정답기호정합·선지5개·**동일문장중복 0·어휘정답 타문항노출 0**. 정답분포 ①5②5③6④5⑤5(균등). 개수형 3문항(#5·#9·#18) 정답개수=해설 일치 재확인.
  - 산출=`/mnt/user-data/outputs/jechuljung3_2026_1f_exam.json`(덮어쓰기). 1차 검수기록은 아래 항목 참조.
- **재검수: jechuljung3_2026_1f_exam.json (업로드본) — 실오류 0건 재확인 (2026-06-29)**
  - 레포 raw에서 handoff.md + exam_authoring_universal_handoff.md + exam_gate.py 규칙 먼저 학습 후 검수.
  - ⚠️ **handoff 5번 기록과 이 업로드본은 발문 형태가 다른 버전**: 기록은 '밑줄형+개수형' 8어법인데, 업로드본은 #4(ⓐ~ⓔ 밑줄형)·#5·#9·#18(개수형)·#7·#14·#15·#16(선지형 8어법) 구성. 업로드본 기준으로 전수검수함.
  - **26문항·100점·번호연속·JSON유효성 PASS.** 정답분포 ①4②4③6④7⑤5.
  - **어법 8문항 핵심4포인트(명사절if/과거완료/분사후치/가목적어it~to) 충실.** 개수형 3문항 정답개수=해설설명 한문장씩 대조 일치(#5=1개©, #9=2개©ⓔ, #18=4개ⓐⓑⓒⓓ). #4 that→it, #7·#14 make/making→made, #15·#16 시제/분사 의도오류-정답 정합.
  - 대화·내용일치·어휘 전문항 정답유일·복수정답 없음. #25 <u>can't wait to</u> 태그 정상.
  - **교차 답노출 점검**: L3 진실(독립운동 후원)을 #1·#2·#10·#19가 공유하나 어느 문항도 그 사실을 정답으로 직접 묻지 않음 → 노출 아님(안전).
  - **검토권장(정답변경 아님)**: ①#5(D) 'Nobody knew if...or not' — if...or not 내신허용이나 엄격기준이면 whether 선호 → 어색 2개 될 여지, 출제자 의도확정 권장(MED). ②정답분포 ④7개 쏠림 — 선지독립 유형 1~2개 재배치로 ④6⑤6 균등화 가능(LOW). ③지문편중 L4 12 > L3 10(LOW).
  - **미적용 보류**: 위 검토권장 3건은 사용자 확답 후 반영 예정(현재 원본 무수정). 산출=업로드 원본 그대로 유지.
