...
6. **밑줄 표기** — "밑줄 친 ~" 발문의 대상 부분은 반드시 `<u>...</u>` 태그로 표기.
   (인쇄·화면 렌더가 HTML을 그대로 표시함. 기호 마킹 ⓐ~ⓔ·①~⑤ 옆 검사 대상도 동일.)
7. **시트 호환** — 시트 `question_list` 열(JSON 문자열) ↔ `questions` 배열은 동일 데이터.
   허브 페이지가 양방향 자동 변환.
8. **파일명** — 영문+숫자만. 예: `jechul2_2026_1f_pattern.json`, `jechul2_2026_1f_exam.json`
